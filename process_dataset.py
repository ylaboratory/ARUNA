...
For e.g., for patch span (s) = 5000, patch id = 1 spans from 1-4999bp on the chromosome.
This span may have 0-5000 CpGs and the start positions of these CpGs are needed.
...
Otherwise a pre-asigned number of workers are used.
"""
...
from patch_utils import get_patch_data
from read_utils import get_processed_df
from noise_simulators import get_sim_mask
from patch_metadata import get_patchPosn_metadata, get_mPatchPosn_metadata
...
def get_cc_gt(dataset, chrom, data_dir = None):
    ...
    if fm_save_path.is_file() and cov_save_path.is_file():
        ...
        if data_dir == None:
            print("Data directory with .cov files not provided! Exiting...")
            sys.exit(0)
...
def get_cc_noisy(dataset, chrom, nr, data_dir = None, seed = None):
    ...
    if not seed:
        warnings.warn("NR seed not specified! Defaulting to older dir naming and if not found, setting seed = 0 to generate data...")
        seed = 0
        save_dir = os.path.join(CWD, "data", dataset, "chrom_centric", nr)

    else:
         save_dir = os.path.join(CWD, "data", dataset, "chrom_centric", nr+"-seed"+str(seed))
...
def get_pc_gt(dataset, chrom, patch_type = "gpatch", 
              s = None, num_cpg = None, data_dir = None, 
              data_type = "fm", subset = None):
    ...
    assert s or num_cpg, "One of Patch Size or Num CpGs need to be supplied!"
    
    if patch_type == "gpatch":
        save_dir = os.path.join(CWD, "data", dataset, "patch_centric", "span"+str(s), "true")
    elif patch_type == "mpatch":
    ...
    for c in chrom:
        ...
        if save_path.is_file():
            ...
            pc_gtfm_map = make_pc_gt(dataset, c, patch_type, 
                                    data_dir, save_path,
                                    s, num_cpg, data_type)
...
def get_pc_noisy(dataset, chrom, patch_type = "gpatch", 
                 s = None, num_cpg = None, nr = "rrbs_sim", 
                 data_dir = None, seed = None, subset = None):
    ...
    if not seed:
        ...
        seed = 0
        if patch_type == "gpatch":
            save_dir = os.path.join(CWD, "data", dataset, "patch_centric", "span"+str(s), nr)
        elif patch_type == "mpatch":
            save_dir = os.path.join(CWD, "data", dataset, "patch_centric", "numCpg"+str(num_cpg), nr)

    else:
        if patch_type == "gpatch":
            save_dir = os.path.join(CWD, "data", dataset, "patch_centric", "span"+str(s), nr+"-seed"+str(seed))
        elif patch_type == "mpatch":
            save_dir = os.path.join(CWD, "data", dataset, "patch_centric", "numCpg"+str(num_cpg), nr+"-seed"+str(seed))
...
def make_cc_gt(dataset, chrom, 
    ...
    if dataset == "gtex":
        ...
        samp_dirs = sorted(
                        ...
                        if os.path.isdir(p))
        # samp_dirs = sorted(os.listdir(data_dir))[:-2]
    
    elif "_".join(dataset.split("_")[:2]) == "encode_wgbs": # for downsampled dirs named like "encode_wgbs_frac0.X_seedY"
         samp_dirs = sorted(
                        os.path.basename(p) for p in glob.glob(os.path.join(data_dir, "ENCFF*")) 
                        if os.path.isdir(p))
    
    elif dataset == "sra_wgbs":
        samp_dirs = sorted(os.listdir(data_dir))[:-1]
    
    elif dataset == "atlas":
        atlas_files = sorted(os.listdir(data_dir))
        samp_dirs = [i for i in atlas_files if i[:4] == "EGAN"]
        assert len(samp_dirs) == 410, "Sample number mismatch for Atlas dataset?"
    else:
...
def make_cc_noisy(dataset, chrom, nr, 
                  hg38_chr_df, chr_fm_df, 
                  fm_save_path, mask_save_path, seed):
    ...
    all_simMiss_fm, all_simMiss_mask = get_noisy_data(nr, chrom, chr_fm_df, 
                                                      hg38_chr_df, seed) # returns 2 dicts
...
def make_pc_gt(dataset, chrom, patch_type, 
               data_dir, save_path, 
               s = None, num_cpg = None, 
               data_type = "fm", num_workers = 32):
    
    if patch_type == "gpatch":
        print("Processing for {} - {} and patch size {}...".format(dataset, chrom, s))
        patch_refCpG_map = get_patchPosn_metadata(chrom, s)[0]
    elif patch_type == "mpatch":
        print("Processing for {} - {} and {} CpGs/Patch...".format(dataset, chrom, num_cpg))
        patch_refCpG_map = get_mPatchPosn_metadata(chrom, num_cpg)
...
def make_pc_noisy(dataset, chrom, 
    ...
    if patch_type == "gpatch":
        print("Processing for {} - {} - {} and patch size {}...".format(dataset, chrom, nr, s))
        patch_refCpG_map = get_patchPosn_metadata(chrom, s)[0]
    elif patch_type == "mpatch":
...
def get_noisy_data(noise_regime, chrom, 
    ...
    for samp in chr_fm_df.columns:
        ...
        sim_mask = get_sim_mask(noise_regime, chrom, hg38_chrcpg_df, seed)
...
# resolve the data dir in scratch
# resolve hg38 data for current chromosome
# This forms the "Canonical" CpG set for all our experiments

# hg38_all_df = pd.read_csv(os.path.join(hg38_metadata_dir, "hg38_cpg_py0idx.csv"))
# hg38_chr_df = hg38_all_df[hg38_all_df.loc[:, "seqname"] == chrom].set_index("start")
# print("Total Hg38 CpGs in {}: {}".format(chrom, hg38_chr_df.shape[0]))