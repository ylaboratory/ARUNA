...
def get_mpatch_dataset(config, samp_split = None, 
                       chr = None, chr_split = None):

    """
    Returns an appropriate dataloader object for the MPatchCentricDataset class.
    Params vary depending if multiple or single chromosome training is used.
    """

    if chr and chr_split:
        raise ValueError("A single chromosome and a chromosome split can not be specified together!")
    ...
    # CASE 1: Multiple chromosomes
    if chr_split:
        train_chr = chr_split["Train"]
        val_chr = chr_split["Validate"]
    # CASE 2: Single chromosome
    else:
        train_chr = chr
        val_chr = chr

    # get patches for all dataset samples for Training chromosomes
    # Redundancy: When train_dataset = test_dataset; train_nr = test_nr and single chr 
    #             (or multiple with no holdout);
    #             gt/nr/simMask_train are the exact same as gt/nr/simMask_val since data
    #             for all samples is supplied.
    
    logger.info("Loading Training Data...")
    gt_train, nr_train, simMask_train = get_mpatches(config["data"]["train_dataset"],
                                                     train_chr,
                                                     config["data"]["kind"],
                                                     config["data"]["num_cpgs"],
                                                     config["exp"]["train_nr"])
    logger.info("Loading Validation Data...")
    # get patches for all dataset samples for Validation chromosomes
    gt_val, nr_val, simMask_val = get_mpatches(config["data"]["test_dataset"],
                                               val_chr,
                                               config["data"]["kind"],
                                               config["data"]["num_cpgs"],
                                               config["exp"]["test_nr"])
    
    trainData_obj = MPatchCentricDataset(train_samps, 
                                         config["data"]["num_cpgs"], 
                                         gt_train, 
                                         nr_train, 
                                         simMask_train,
                                         config["model"]["posn_embed"],
                                         transform = transforms_list)     

    valData_obj = MPatchCentricDataset(val_samps, 
                                        config["data"]["num_cpgs"], 
                                        gt_val, 
                                        nr_val, 
                                        simMask_val,
